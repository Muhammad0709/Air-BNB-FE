...
import { Box, Breadcrumbs, Button, Paper, Typography, FormControlLabel, Checkbox, Stack, TextField, Alert } from '@mui/material'
...
export default function ListingDetail() {
  ...
  // Airport pickup service state
  const [pickupServiceEnabled, setPickupServiceEnabled] = useState(false)
  ...
  const propertyData = {
    ...
    airportFare: 30,
    serviceStartTime: '10:00', // 10 AM
    serviceEndTime: '21:00', // 9 PM
  }
  ...
  const handlePickupServiceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const checked = e.target.checked
    setPickupServiceEnabled(checked)
    if (!checked) {
      setPickupTime(null)
      setPickupTimeError('')
    }
  ...
  const handlePickupTimeChange = (newValue: Dayjs | null) => {
    ...
    if (newValue && propertyData.airportPickupEnabled) {
      const [startHour, startMin] = propertyData.serviceStartTime.split(':').map(Number)
      const [endHour, endMin] = propertyData.serviceEndTime.split(':').map(Number)
      
      const selectedHour = newValue.hour()
      const selectedMinute = newValue.minute()
      
      const minTime = dayjs().hour(startHour).minute(startMin)
      const maxTime = dayjs().hour(endHour).minute(endMin)
      const selectedTime = dayjs().hour(selectedHour).minute(selectedMinute)
      
      // Check if selected time is within service hours
      if (selectedTime.isBefore(minTime) || selectedTime.isAfter(maxTime)) {
        setPickupTimeError(`Service is only available from ${minTime.format('h:mm A')} to ${maxTime.format('h:mm A')}`)
      } else {
        setPickupTimeError('')
      }
    }
  ...
  const minTime = propertyData.airportPickupEnabled 
    ? dayjs().hour(parseInt(propertyData.serviceStartTime.split(':')[0])).minute(parseInt(propertyData.serviceStartTime.split(':')[1]))
    : dayjs().hour(10).minute(0)
  const maxTime = propertyData.airportPickupEnabled
    ? dayjs().hour(parseInt(propertyData.serviceEndTime.split(':')[0])).minute(parseInt(propertyData.serviceEndTime.split(':')[1]))
          ...
          <RBContainer className="mt-4">
            <Row>
              <Col lg={12}>
                  ...
                  <Paper className="amenities-section" elevation={0}>
                    ...
                    <Row className="g-3">
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <WifiIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                      ...
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <AcUnitIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                    ...
                    <Row className="mt-3">
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <KitchenIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                      ...
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <LocalParkingIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                    ...
                    <Row className="mt-3">
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <PoolIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                      ...
                      <Col md={6} sm={6}>
                        <Box className="info-item d-flex gap-2" sx={{ alignItems: 'center', mb: 2 }}>
                          <Box className="info-icon" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mr: 1.5 }}>
                            <PetsIcon sx={{ color: '#FF8C75', fontSize: '24px', width: '24px', height: '24px' }} />
                          </Box>
                ...
                {propertyData.airportPickupEnabled && (
                  <Paper className="pickup-service-section mt-4" elevation={0} sx={{ p: 3, border: '1px solid #E5E7EB', borderRadius: 2 }}>
                    <Typography className="section-title" component="h2" sx={{ mb: 2 }}>Airport Pickup Service</Typography>
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="body2" sx={{ color: '#6B7280', mb: 1 }}>
                        <strong>Airport:</strong> {propertyData.airport}
                      </Typography>
                      <Typography variant="body2" sx={{ color: '#6B7280', mb: 1 }}>
                        <strong>Fare:</strong> ${propertyData.airportFare}
                      </Typography>
                      <Typography variant="body2" sx={{ color: '#6B7280' }}>
                        <strong>Service Hours:</strong> {minTime.format('h:mm A')} - {maxTime.format('h:mm A')}
                      </Typography>
                    </Box>
                    ...
                    <FormControlLabel
                      ...
                      label="Would you like to book a pick up service?"
                      sx={{ mb: 2 }}
                    />
                    
                    {pickupServiceEnabled && (
                      <LocalizationProvider dateAdapter={AdapterDayjs}>
                        <Box sx={{ mt: 2 }}>
                          <TimePicker
                            ...
                            slotProps={{
                              textField: {
                                ...
                                helperText: pickupTimeError || 'Select your preferred pickup time',
                                sx: {
