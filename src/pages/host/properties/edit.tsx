...
export default function EditProperty() {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      // amenities: mockProperty.amenities // eslint-disable-line
      airportPickupEnabled: mockProperty.airportPickupEnabled,
      airport: mockProperty.airport,
      airportFare: mockProperty.airportFare,
      serviceStartTime: mockProperty.serviceStartTime,
      serviceEndTime: mockProperty.serviceEndTime,
    })
    // setCustomAmenities(existingCustomAmenities)
    
    // Set time picker values
    const [startHour, startMin] = mockProperty.serviceStartTime.split(':').map(Number)
    const [endHour, endMin] = mockProperty.serviceEndTime.split(':').map(Number)
    setStartTime(dayjs().hour(startHour).minute(startMin))
    setEndTime(dayjs().hour(endHour).minute(endMin))
  }, [id])
